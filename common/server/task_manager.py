...
class TaskManager(ABC):
    ...
    async def on_send_task_subscribe(self, request: SendTaskStreamingRequest
                                     ) -> Union[AsyncIterable[SendTaskStreamingResponse],
    JSONRPCResponse]:
        pass
    ...
    @abstractmethod
    async def on_resubscribe_to_task(self, request: TaskResubscribeRequest) -> Union[
...
class InMemoryTaskManager(TaskManager):
    ...
    async def on_get_task_push_notification(self,
        ...
        try:
            push_notification_config = await self.get_push_notification_info(task_id_params.id)
        except Exception as e:
        ...
        return GetTaskPushNotificationResponse(id=request.id, result=TaskPushNotificationConfig(
    ...
    async def upsert_task(self, task_send_params: TaskSendParams) -> Task:
        ...
        async with self.lock:
            ...
            if task is None:
                task = Task(id=task_send_params.id,
    ...
    async def on_resubscribe_to_task(self,
                                     request: TaskResubscribeRequest) -> Union[
        AsyncIterable[SendTaskStreamingResponse], JSONRPCResponse]:
        return new_not_implemented_error(request.id)

    async def update_store(self,
                           task_id: str, status: TaskStatus, artifacts: list[Artifact]) -> Task:
        async with self.lock:
            try:

                task = self.tasks[task_id]
            except KeyError:
                logger.log(f"Task {task_id} not found for updating the task")
                raise ValueError(f"Task {task_id} not found")
    ...
    async def setup_sse_consuber(self, task_id: str, is_resubscribe: bool = False):
        async  with self.subscriber_lock:
            if task_id not in self.task_sse_subscribers:
                if is_resubscribe:
                    raise ValueError("Task not found for resubsription")
                else:
                    self.task_sse_subscribers[task_id] = []

            ss_event_queue = asyncio.Queue(maxsize=0)
            self.task_sse_subscribers[task_id].append(ss_event_queue)
            return ss_event_queue
    ...
    async def enqueue_events_for_sse(self, task_id, task_update_event):
        async with self.subscriber_lock:
            ...
            for sse_event_queue in current_subscribers:
                await sse_event_queue.put(task_update_event)

    async def deque_events_for_sse(self,
                                   request_id, task_id, sse_event_queue: asyncio.Queue) -> AsyncIterable[
