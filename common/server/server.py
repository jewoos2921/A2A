...
class A2AServer:
    ...
    async def _process_request(self, request: Request):
        try:
            ...
            json_rpc_request = A2ARequest.validate_json(body)
            ...
            if isinstance(json_rpc_request, GetTaskRequest):
                ...
                logger.warning(f"Unknown request type: {type(json_rpc_request)}")
    ...
    def _create_response(self, result: Any) -> JSONResponse | EventSourceResponse:
        if isinstance(result, AsyncIterable):
            ...
            logger.error(f"Unknown response type: {type(result)}")
            raise ValueError(f"Unknown response type: {type(result)}")
